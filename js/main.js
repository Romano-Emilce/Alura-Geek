import { listarProductos, enviarProducto, borrarProducto } from "./api.js";
...
// Función asincrónica para crear un producto
async function crearProducto(evento) {
    ...
    try {
        // Simulación de conexiónApi.enviarProducto(nombre, precio, imagen);
        // Aquí deberías tener la lógica real para enviar los datos al servidor
        console.log(`Enviando producto: ${nombre}, ${precio}, ${imagen}`);

        // Limpiar el formulario después del envío exitoso
        formulario.reset();

        // Recargar la página (esto podría ser opcional)
...
// Función para crear un elemento figure dinámicamente
function crearFigure(id, nombre, precio, imagen) {
    const card = document.createElement("figure");
    card.className = "figure";

    card.innerHTML = `
        <div>
            <img src="${imagen}" alt="Dibujo de " class="border border-radius bg-white w-medium" id="${id}">
        </div>
        <div class="figure-text">
            <p class="w-color ibm-plex-mono-regular" id="${id}">${nombre}</p>
            <div class="figure-price" id="${id}">
                <p class="w-color ibm-plex-mono-bold">$ ${precio}</p>
                <button class="bt-eliminar">
    ...
    botonEliminar.addEventListener("click", async () => {
        try {
            // Simulación de conexionApi.borrarProducto(id)
...
// Función principal para listar productos desde la API
async function listarFigure() {
    try {
        // Simulación de conexionApi.listarProductos()
        const listaApi = [
            {
                "id": 1,
                "nombre": "Cocodrilo",
                "precio": 60,
                "imagen": "img/cocodrilo.png"
              },
              {
                "id": 2,
                "nombre": "Pato",
                "precio": 40,
                "imagen": "img/dibujos-animados.png"
              },
              {
                "id": 3,
                "nombre": "Cohete",
